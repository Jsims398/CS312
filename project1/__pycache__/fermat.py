...
def mod_exp(x: int, y: int, N: int) -> int: #This function has a complexity of O(log(n)) due to the recursive call
    if(y == 0):  
        return 1
    z = mod_exp(x, y//2, N)
    if(y % 2 == 0):
...
# hi, inclusive.
def fermat(N: int, k: int) -> str: #This complexity will be O(k*log(N)^3) which due to bits translates to O(N^4)
    for i in range(k):
...
# random.randint(low, hi) which gives a random integer between low and
# hi, inclusive.
def miller_rabin(N: int, k: int) -> str: #This complexity will be O(k*log(N)^3) which due to bits translates to O(N^4)
    if N <= 1 or N % 2 == 0:
        return "composite" #base case for N
    if N == 2: #base case for N
        return "prime"
        
    for i in range(k): #loop for k times
        a = random.randint(2, N-1)
        y = N - 1
        while y % 2 == 0: #loop until odd
            y //= 2
            
        x = mod_exp(a, y, N)
        if x == 1 or x == N - 1: #prime base case 
            continue
            
        while y != N - 1: #square until N-1
            x = (x**2) % N 
            y *= 2
            if x == 1:
                return "composite"
